...
class ReportePHController extends Controller
{
    ...
    public function index(){
    	...
    	return view('admin.reportePH.index')
    ...
    public function store(ReporteHRequest $request){
    	try{    		
            ...
            if(strcmp($request->input('articulo'),"todo") !== 0){
    		    ...
    		    $view = \View::make('admin.reportePH.create', compact('kardex','fechaImpresion','fechaIncial','fechaFinal','arrayCabecera'))->render();
                ...
                $view = \View::make('admin.reportePH.createFull', compact('cabecera','fechaImpresion','fechaIncial','fechaFinal','nuevaFechaStart','nuevaFechaEnd'))->render();
                $pdf = \App::make('dompdf.wrapper');
                $pdf->setPaper('Letter','portrait');
                $pdf->loadHTML($view);
                return $pdf->download('KARDEX'.date('d-m-Y').date('H-i-s').'.pdf');
    		...
    		return redirect()->route('reporteph.index');
    	}
    }
